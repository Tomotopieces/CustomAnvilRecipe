    ...
    public CreateGui openGui(Player player) {
        ...
        player.openInventory(Optional.of(Bukkit.createInventory(player, 9, GUI_NAME)).map(inv -> {
            Stream.iterate(0, i -> i + 1)
                    ...
                    .forEach(i -> {
                        switch (i) {
                            case 0: inv.setItem(i, Optional.of(new ItemStack(Material.MAP))
                                    .map(bb -> setButton(bb, SAVE_BUTTON_NAME, "§r§eClick to return to menu.")).get());
                                return;
                            case 5: inv.setItem(i, Optional.of(new ItemStack(Material.GLASS_BOTTLE))
                                ...
                                return;
                            case 8: inv.setItem(i, Optional.of(new ItemStack(Material.BOOK))
